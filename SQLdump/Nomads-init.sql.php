<?php
/*
Created 2019-01-02 by Karsten Maske
from initial Nomads.sql
Included during install.
!! Changed structure of table `game_settings`.

Tables:
 galactic_market_buy_table
 game_settings
 map_default
 map_generated
 planet_generated
...
$DB->query('DROP TABLE IF EXISTS `galactic_market_buy_table`;');
$DB->query('CREATE TABLE `galactic_market_buy_table` (
...
$DB->query("INSERT INTO `galactic_market_buy_table` (`listing_id`, `listing_model`, `listing_table`, `listing_value`, `listing_currency`, `listing_limit`, `listing_active`) VALUES
...
/*
OLD:
$DB->query('CREATE TABLE `game_settings` (
...
$DB->query("INSERT INTO `game_settings` (`game_setting_ID`, `game_setting_name`, `game_setting_value`) VALUES
...
$DB->query("CREATE TABLE `game_settings` (
  `k` varchar(100) DEFAULT '' PRIMARY KEY,
  `v` varchar(100) DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=latin1;");

$DB->query("INSERT INTO `game_settings` VALUES
('user_session_validity_minutes', '60'),
('number_of_default_map_tiles', '50'),
('star_spawn_percentage', '4'),
('star_gravity_factor', '1000'),
('number_of_default_star_models', '4'),
('number_of_default_planet_models', '4');");




$DB->query('DROP TABLE IF EXISTS `map_default`;');
$DB->query('CREATE TABLE `map_default` (
...
$DB->query("INSERT INTO `map_default` (`map_id`, `map_name`, `map_X`, `map_Y`, `map_star`, `map_image`, `map_tile`) VALUES
(1, 'Solar System', 0, 0, 1, '0', 'solarsystem_1');");


$DB->query('DROP TABLE IF EXISTS `map_generated`;');
$DB->query('CREATE TABLE `map_generated` (
...
$DB->query('DROP TABLE IF EXISTS `planet_generated`;');
$DB->query('CREATE TABLE `planet_generated` (
...
$DB->query('DROP TABLE IF EXISTS `planet_model`;');
$DB->query('CREATE TABLE `planet_model` (
...
$DB->query("INSERT INTO `planet_model` (`model_id`, `model_name`, `model_diameter_range`) VALUES
(1, 'typeA', '10000;15000'),
(2, 'typeB', '5000;7500'),
...
$DB->query('DROP TABLE IF EXISTS `star_default`;');
$DB->query('CREATE TABLE `star_default` (
...
$DB->query("INSERT INTO `star_default` (`star_id`, `star_name`, `star_diameter`, `star_heat`, `star_gravity`, `star_map`, `star_model`, `star_image`) VALUES
(1, 'Sun', 1300000, 5500, 130, 1, 1, 'default_sun');");


$DB->query('DROP TABLE IF EXISTS `star_generated`;');
$DB->query('CREATE TABLE `star_generated` (
...
$DB->query('DROP TABLE IF EXISTS `star_model`;');
$DB->query('CREATE TABLE `star_model` (
  `model_id` int unsigned auto_increment PRIMARY KEY,
...
$DB->query("INSERT INTO `star_model` (`model_id`, `model_name`, `model_diameter_range`, `model_heat_range`, `model_gravity_range`) VALUES
(1, 'Red Giant', '1000000;1500000', '5000;6000', '100;150');");


$DB->query('DROP TABLE IF EXISTS `unit_model_table`;');
$DB->query('CREATE TABLE `unit_model_table` (
  `model_id` int unsigned auto_increment PRIMARY KEY,
...
$DB->query("INSERT INTO `unit_model_table` (`model_id`, `model_name`, `model_type`, `model_description`, `model_speed`, `model_hitpoints`, `model_attack`, `model_cargo`, `model_require_workers`, `model_active`) VALUES
(1, 'Mother Ship', 'ship', 'description_ship_1', 1, 1000000, 0, 15000, 100, 1),
(2, 'Explorer Probe', 'ship', 'description_ship_2', 100, 1, 0, 0, 0, 1);");


$DB->query('DROP TABLE IF EXISTS `unit_table`;');
$DB->query('CREATE TABLE `unit_table` (
...
$DB->query('DROP TABLE IF EXISTS `user_session`;');
...
$DB->query('DROP TABLE IF EXISTS `user_token`;');
$DB->query('CREATE TABLE `user_token` (
...
$DB->query('/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;');
$DB->query('/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;');
$DB->query('/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;');
?>